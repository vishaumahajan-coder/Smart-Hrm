...
const CompanySelection = ({ onSelect, onExit }) => {
    ...
    return (
        <div className="w-[600px] bg-[#EBE9D8] border-2 border-[#0055E5] shadow-2xl flex flex-col font-sans">
            {/* Title Bar */}
            <div className="bg-[#0055E5] text-white px-2 py-1 flex justify-between items-center text-xs font-bold shadow-[inset_0_1px_0_rgba(255,255,255,0.3)]">
                <div className="flex items-center gap-1">
                    <div className="w-4 h-4 bg-white/20 rounded-sm italic flex items-center justify-center text-[10px]">C</div>
                    Company Selection
                </div>
                <button
                    onClick={onExit}
                    className="bg-[#E81123] hover:bg-red-600 text-white w-4 h-4 flex items-center justify-center text-[10px] border border-white/20"
                >
                    ✕
                </button>
            </div>
                    ...
                    <table className="w-full border-collapse text-xs">
                        ...
                        <tbody>
                            {companies.map((company) => (
                                ...
                                >
                                    <td className="px-2 py-1 border-r border-gray-100 uppercase font-medium">{company.name}</td>
                                    <td className="px-2 py-1 uppercase font-medium">{company.frequency}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
            ...
            {/* status bar inside selection if needed, but App.jsx has one global */}
        </div>
    );
};
